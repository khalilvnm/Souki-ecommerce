...
//           <div>
//             <span className="text-red-700 font-semibold mr-3">{discount} Off</span><span className="font-semibold">{calculateProductDiscount(price, discount)}{currency}</span>
//             <p className="line-through text-gray-600 font-semibold">{price}{currency}</p>
//           </div>
//         </div>
...
const ProductItem = ({ id, title, images, price, discount }) => {
  ...
  return (
    <div className="bg-white block border p-3 border-gray-200 rounded-xl shadow-lg cursor-pointer transition-all duration-300">
      <div className="relative">
        <div 
          onClick={() => { addAndRemoveWishList(id); }}
          className={`absolute flex items-center justify-center w-6 h-6 cursor-pointer top-[-5px] right-[-5px] ${wishlistItems.includes(id) ? "text-red-700" : "text-gray-700"}`}
        > 
          <FaStar /> 
        </div>
        <img src={images[0]} alt="product-image" className="h-[180px] mx-auto" />
